...
async function getBaseUrl(): Promise<string> {
  // In server components we can derive the origin from the incoming request
  ...
  // Fallbacks for CI / build time where there is no incoming request
  return (
        ...
        {images.length === 0 ? (
          ...
          <div className='columns-2 sm:columns-3 lg:columns-4 gap-4 space-y-4'>
            {images.map((img) => (
              <div
                key={img.id}
                className='break-inside-avoid rounded-xl overflow-hidden border border-zinc-800/50 bg-zinc-900/40 group'
              >
